emar:/ˈiːmɑːr/ | Not Found in Standard Dictionaries |  Not Found | This word does not appear in standard English dictionaries. It could be a misspelling, a proper noun (like a company name), an abbreviation, or a word from a specialized field.  Without more context, it's impossible to provide a meaningful definition. |  N/A | N/A | N/A | N/A

Since "emar" isn't a recognized word, I can't provide examples, synonyms, antonyms, or etymological information. However, I can still demonstrate the requested grammar points using the word "what" as requested by the prompt's instructions concerning  the "USAGE" section, which seems to be the main focus of the request.

USAGE->

一般现在时 (Simple Present):
What is your name? = 你叫什么名字？

What does he do for a living? = 他以什么为生？


现在进行时 (Present Continuous):
What are you doing now? = 你现在在做什么？

What is she cooking for dinner? = 她正在做什么晚餐？


现在完成时 (Present Perfect):
What have you done? = 你做了什么？

What has she eaten today? = 她今天吃了什么？


现在完成进行时 (Present Perfect Continuous):
What have you been doing all day? = 你一整天都在做什么？

What has he been working on lately? = 他最近一直在做什么？


一般过去时 (Simple Past):
What did he say? = 他说了什么？

What happened yesterday? = 昨天发生了什么事？


过去进行时 (Past Continuous):
What were you doing when I called? = 我打电话时你在做什么？

What was she thinking about? = 她当时在想什么？


过去完成时 (Past Perfect):
What had happened before I arrived? = 在我到达之前发生了什么事？

What had they discussed before the meeting? = 会议前他们讨论了什么？


过去完成进行时 (Past Perfect Continuous):
What had you been doing before the accident? = 事故发生前你一直在做什么？

What had they been discussing for so long? = 他们一直在讨论什么？


一般将来时 (Simple Future):
What will you do tomorrow? = 你明天要做什么？

What will the weather be like next week? = 下周天气会怎样？


将来进行时 (Future Continuous):
What will you be doing this time tomorrow? = 明天这个时候你会在做什么？

What will they be discussing at the meeting? = 会议上他们会讨论什么？


将来完成时 (Future Perfect):
What will you have accomplished by next year? = 到明年你会完成什么？

What will have changed by then? = 到那时会有什么变化？


将来完成进行时 (Future Perfect Continuous):
What will you have been doing for ten years? = 到时候你将已经做这件事十年了？

What will she have been studying by graduation? = 到毕业时她将已经学习什么专业多久了？



PHRASE->
What's up? = 怎么了？/ 你好吗？

What the hell! = 搞什么鬼！

what if = 如果...会怎么样

what for = 为了什么

no matter what = 无论如何


注意事项:  As mentioned, "emar" is not a recognized word.  Please double-check the spelling or provide more context.
